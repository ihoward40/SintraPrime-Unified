        ...
        try:
            while not token.is_expired():
                if token.should_refresh():
                    ...
                    retry_delay = 1 if success else min(retry_delay * 2, 60)  # reset / exponential backoff
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if is_protected:
            ...
            if not session_id:
                ...
                return JSONResponse(status_code=401, content={"detail": "Unauthorized: No session"})
