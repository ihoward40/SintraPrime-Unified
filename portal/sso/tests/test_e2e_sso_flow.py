...
class TestRedisSessionStore:
    ...
    async def test_store_and_retrieve(self):
        ...
        mock_client.get = AsyncMock(
            return_value=json.dumps({"user_id": "u123", "email": "test@example.com"})
        )
        store._client = mock_client
        store._connected = True
        result = await store.store(
            "sess_abc", {"user_id": "u123", "email": "test@example.com"}, ttl_seconds=3600
        )
...
class TestE2ESSOFlow:
    ...
    async def test_session_expiry(self):
        ...
        mgr = SessionMiddlewareManager(
            session_secret="test-secret-32chars-minimum!!", session_ttl_seconds=1
        )
