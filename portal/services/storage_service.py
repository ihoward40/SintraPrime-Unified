...
class StorageService:
    ...
    async def upload_file(
        ...
        """Alias for put_object, returns the key."""
        await self.put_object(bucket=bucket, key=key, data=data, content_type=content_type, metadata=metadata)
        return key

    async def generate_presigned_url(self, bucket: str, key: str, expires: int = 3600) -> str:
        """Alias for presigned_get_url."""
        return await self.presigned_get_url(bucket=bucket, key=key, expires_seconds=expires)
