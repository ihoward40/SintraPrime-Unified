...
The review-driven gaps are closed at the specification, schema, and scaffold layer: note versioning and downgrade rules; write-once source semantics; first-class contradiction records; quarantine exit authority; concrete profile ACLs; explicit profile switching; fail-closed receipt behavior; parser-cache recovery; physical tenant segregation; dry-run behavior; sensitivity-filtered queries; append-only chained receipts; inline schema versions; additive migrations; and an explicit merge-certification boundary. Runtime enforcement remains gated to Increment 2 certification.
