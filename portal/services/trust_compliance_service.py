...
class TrustComplianceService:
    def __init__(self):
        self.risk_tags = self._load_policy_data(
            "apps/sintraprime/src/modules/trust-compliance/policies/risk-tags.ts",
            "RISK_TAG_DEFINITIONS",
        )
        self.safety_gates = self._load_policy_data(
            "apps/sintraprime/src/modules/trust-compliance/policies/safety-gates.ts",
            "SAFETY_GATE_DEFINITIONS",
        )
        self.forbidden_phrases = self._load_policy_data(
            "apps/sintraprime/src/modules/trust-compliance/policies/forbidden-phrases.ts",
            "FORBIDDEN_PHRASES",
        )
    ...
    def _load_policy_data(self, file_path: str, var_name: str) -> List[str]:
        ...
        if var_name == "RISK_TAG_DEFINITIONS":
            match = re.search(
                r"export const RISK_TAG_DEFINITIONS: Record<RiskTag, string> = {([^}]+)};",
                content,
                re.DOTALL,
            )
            if match:
            ...
            always_block_tags_match = re.search(
                r"const ALWAYS_BLOCK_TAGS: RiskTag\[\] = \[(\s*\'[^\]]+\'(?:,\s*\'[^\]]+\')*\s*)\];",
                content,
            )
            if always_block_tags_match:
                policies.extend(
                    [
                        item.strip().strip("'")
                        for item in always_block_tags_match.group(1).split(",")
                        if item.strip()
                    ]
                )
            rewrite_tags_match = re.search(
                r"const REWRITE_TAGS: RiskTag\[\] = \[(\s*\'[^\]]+\'(?:,\s*\'[^\]]+\')*\s*)\];",
                content,
            )
            if rewrite_tags_match:
                policies.extend(
                    [
                        item.strip().strip("'")
                        for item in rewrite_tags_match.group(1).split(",")
                        if item.strip()
                    ]
                )
        elif var_name == "FORBIDDEN_PHRASES":
            match = re.search(
                r"export const FORBIDDEN_PHRASES: ForbiddenPhrase\[\] = \[(\s*{[^}]+}(?:,\s*{[^}]+})*\s*)\];",
                content,
                re.DOTALL,
            )
            if match:
                ...
                policies = [
                    phrase.strip().strip("'")
                    for phrase in re.findall(r"phrase: \'([^\']+)\'", phrases_content)
                ]
        return policies
    ...
    def analyze_document(self, document_text: str, document_type: str) -> Dict:
        ...
        compliance_score = (
            1.0 - (len(triggered_risk_tags) * 0.1) - (len(forbidden_phrases_found) * 0.15)
        )
        ...
        if triggered_risk_tags:
            recommendations.append(
                f"Address triggered risk tags: {', '.join(triggered_risk_tags)}."
            )
        if forbidden_phrases_found:
            recommendations.append(
                f"Remove or rephrase forbidden phrases: {', '.join(forbidden_phrases_found)}."
            )
