...
from ..services.case_security import CaseSecurityError, verify_signed_internal_event
from ..services.case_starter_service import case_starter_service
...
@router.post(
    "/monetization/start-case",
    response_model=StartCaseResponse,
    summary="Start a new case after payment",
    responses={
        200: {"description": "Case started and drafts generated"},
        400: {"description": "Invalid request"},
        409: {"description": "Case ID already exists"},
        500: {"description": "Failed to start case"},
    },
)
async def monetization_start_case(
    request: Request,
    payload: StartCaseRequest,
    x_start_case_key_id: str = Header(..., alias="X-Start-Case-Key-Id"),
    x_start_case_timestamp: str = Header(..., alias="X-Start-Case-Timestamp"),
    x_start_case_event_id: str = Header(..., alias="X-Start-Case-Event-Id"),
    x_start_case_signature: str = Header(..., alias="X-Start-Case-Signature"),
):
    """Start a case from a signed normalized internal payment event.

    The signed body is the authorization boundary: it must contain a strict
    gateway-normalized payment event plus the intake fields used to generate the
    case. The upstream gateway verifies Stripe first; this route verifies the
    gateway HMAC, expiry, key id, payment state, amount, currency, and bindings
    before filesystem or ledger side effects.
    """

    try:
        body = await request.body()
        verified_event = verify_signed_internal_event(
            body=body,
            key_id=x_start_case_key_id,
            timestamp=x_start_case_timestamp,
            event_id=x_start_case_event_id,
            signature=x_start_case_signature,
        )
        if payload.case_id != verified_event.case_id:
            raise CaseSecurityError("request case_id must match signed event case_id")
        if payload.tier.value != verified_event.tier:
            raise CaseSecurityError("request tier must match signed event tier")
        if payload.payment_session_id != verified_event.payment_session_id:
            raise CaseSecurityError("request payment_session_id must match signed event payment_session_id")
        return await case_starter_service.start_case(
            payload,
            verified_event=verified_event,
        )
    except CaseSecurityError as e:
        ...
        raise HTTPException(status_code=500, detail="Failed to start case") from e
