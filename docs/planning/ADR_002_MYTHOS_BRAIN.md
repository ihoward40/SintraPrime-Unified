...
- **Retry Policy:** The Brain implements exponential backoff for transient executor failures, governed by a `max_retries` policy per intent type.

### 2.4 Cancellation Primitives
The Brain provides scoped cancellation to ensure system safety:
- **Global Halt:** Immediate suspension of all non-critical execution across the tenant.
- **Workstream Cancellation:** Halts a specific causation chain (e.g., "Stop this specific legal research loop").
- **Executor Revocation:** Signals a specific agent or worker to checkpoint and terminate immediately.
- **Safety Gate:** Cancellation signals are prioritized in the message bus to bypass execution queues.
