...
class StripeClient:
    ...
    async def get_or_create_customer(
        email: str, name: Optional[str] = None, metadata: Optional[Dict[str, str]] = None
    ) -> Customer:
    ...
    async def upgrade_subscription(subscription_id: str, new_tier: str) -> Subscription:
        ...
        try:
            ...
            return Subscription(
                ...
                trial_start=datetime.fromtimestamp(upgraded.trial_start)
                if upgraded.trial_start
                else None,
                trial_end=datetime.fromtimestamp(upgraded.trial_end)
                if upgraded.trial_end
                else None,
    ...
    async def cancel_subscription(
        subscription_id: str, at_period_end: bool = False
    ) -> Subscription:
        ...
        try:
            ...
            return Subscription(
                ...
                trial_start=datetime.fromtimestamp(canceled.trial_start)
                if canceled.trial_start
                else None,
                trial_end=datetime.fromtimestamp(canceled.trial_end)
                if canceled.trial_end
                else None,
                cancel_at=datetime.fromtimestamp(canceled.cancel_at)
                if canceled.cancel_at
                else None,
                canceled_at=datetime.fromtimestamp(canceled.canceled_at)
    ...
    async def create_checkout_session(
        ...
        try:
            ...
            session = stripe.checkout.Session.create(
                ...
                or f"{API_BASE_URL}/dashboard?session_id={{CHECKOUT_SESSION_ID}}",
    ...
    async def refund_payment(
        payment_intent_id: str, amount: Optional[int] = None, reason: str = "requested_by_customer"
    ) -> Payment:
        """Refund a payment"""
        try:
            ...
            refund = stripe.Refund.create(
                payment_intent=payment_intent_id, amount=amount, reason=reason
            )
